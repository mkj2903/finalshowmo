...
// ✅ CREATE ORDER (Public endpoint) - UPDATED WITH COUPON SUPPORT
router.post('/', async (req, res) => {
  try {
    ...
    // ✅ COUPON VALIDATION AND INCREMENT USAGE
    let finalCouponCode = '';
          ...
          if (now >= startDate && now <= endDate) {
            if (coupon.usedCount < coupon.totalQuantity) {
              ...
              if (orderAmount >= coupon.minOrderAmount) {
                ...
                finalCouponDiscount = couponDiscount || 0;
                ...
                console.log(`⚠️ Coupon ${couponCode} requires minimum order of ₹${coupon.minOrderAmount}`);
              }
            } else {
              console.log(`⚠️ Coupon ${couponCode} usage limit reached`);
            }
          } else {
            console.log(`⚠️ Coupon ${couponCode} is expired or not yet active`);
          }
        } else {
